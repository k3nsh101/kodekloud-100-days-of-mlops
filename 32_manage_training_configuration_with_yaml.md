...
- Update the `/root/code/fraud-detection/scr/models/train.py`
  Add random state to `train_test_split` and `RandomForestClassifier`
...
- Run the `check_determinism` script

  ```bash
  /root/code/fraud-detection/check_determinism.sh
  ```

- Verify the results as in step 4
